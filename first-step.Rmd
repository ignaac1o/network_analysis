...
On the other hand, the analysis of the fit of a regression model for the weighted degree distribution is completely different. Here, there are more weights with lower degrees, therefore the slope will be negative and the trend will be descending.
...
plot(weighted_degree_dist_pos-1,
     ...
     log="xy",col=color_1,main="Log-log weighted degree distribution of bicimad network",
...
plot(log_weighted_deg,log_weighted_deg_dist,
     col=color_1,main="Log-log weighted degree distribution of karate network",
     xlab="Log-Weighted-Degree",ylab="Log-Frequency",pch=19)
abline(second_fit$coefficients[1],second_fit$coefficients[2],col=color_2,lwd=2)
```
...
close_W_total <- cbind("Weighted closenesses" = V(bicimad)$address[close_measure$ix][1:5])
knitr::kable(close_W_total)
```

The weighted case shows also that **Calle Manuel Silvela nº 20**, **Calle José Ortega y Gasset nº 4** and **Calle Juan Bravo nº 50** are among the five most central stations.
...
V(bicimad)$address[c(208,80,90,250,58,61,38,250,57)]
```
