...
und_betw_edge <- V(bicimad)[which(V(bicimad)$name %in% c(268,80,90,250,58,61,38,250,57))]$address
und_betw_edge
```
...
dir_betw_edge <- V(bicimad)[which(V(bicimad)$name %in% c(178,87,79,165,132,77,177,256,120,266))]$address
dir_betw_edge
```

We can see that there is a large difference between edge relevance for both scenarios and it does not appear any of the most central vertices calculated before.

## Analyze the local density of the network

To Analyze it we will go through different steps and we will be getting different insights that will help at the end to have a better understanding of our network.

### Calculate cliques

We will first calculate the cliques of the network. As this is quite a big network, it takes a little bit of time to compute all the cliques. However, within less than a minute we can have them.
...
As expected the density is only 0.14, which is a low number, meaning that the network has low density. This result was expected as the records taken for building this network were only for 1 month. If we would have taken a year, the network will be much dense than now.
...
Indeed, we can see that those nodes that were part of the longest path calculated in the first step, "Pedro Rico" and "Hacienda de los Pavones" are the ones with higher density. It might look at first that the results obtained are not correct and that those stations with higher degree should be the more dense ones. However, that logic is indeed wrong as the calculation of the degree considered the weight and therefore the number of times that a link was repeated whereas for the density we are just counting if a node has a link or not with another.
...
Even though there are no different components in our network we can still calculate the mean length of all the shortest paths in this main component.

```{r}
mean_distance(bicimad)
```

We can see that the mean length is 3, which isn't a very large number.

To analyze the connectivity of our network we have to see both its vertex connectivity and edge. For that we will compute its k-vertex and k-edge connectivity. 

```{r}
vertex_connectivity(bicimad)
edge_connectivity(bicimad)
```
...
```
As we can see in the previous figures of the table, the method *label propagation* is not doing properly the partitioning since the modularity is close to zero. The best community detection method among others is the *fast greedy* because it has the largest modularity, the next one is the *Louvain* algorithm. The *fast greedy* choice makes sense. Despite all of this, there is a clear interpretation for all the remaining algorithm that is the clustering has been made based on the geographical areas where a trip starts and ends.
...
## Assess the number of communities in your network using classical and generalized random graph models.

### Classical random graph models

In this section, we will generate random network based on two different classical models: **Erdös and Rényi model** and  **Gilbert model**.

For this purpose, we need to feed the methods that generate the random graphs with the actual number of vertices and edges of our network. Let's generate the random graph with the first approach: Erdös and Rényi model.

```{r}
set.seed(10)
N <- 262
L <- 10000
random_graph_ER <- sample_gnm(n=N,m=L)
random_graph_ER
```

We plot the network obtained with circular layout.

```{r}
V(random_graph_ER)$color <- "deepskyblue2"
plot.igraph(random_graph_ER,layout=layout_in_circle,
            main="Erdös and Rényi network",
            vertex.size=8,vertex.label.cex=0.6)
```

We can see that the network is such dense that we can appreciate the connections. 

Now, we obtain the degree distribution of the generated network in order to compare it with the real degree distribution of our network.

```{r}
deg_ER <- degree(random_graph_ER)
deg_dis_ER <- degree_distribution(random_graph_ER)
par(mfrow=c(2,2))
plot(0:max(deg_ER),deg_dis_ER,col="deepskyblue2",
     main="D. distribution of ER network",
     xlab="Degree",ylab="Frequency",type="h")
points(0:max(deg_ER),deg_dis_ER,pch=20)

plot(0:max(degree(bicimad)),degree_dist,col="deepskyblue2",
     main="D. distribution of real network",
     xlab="Degree",ylab="Frequency",type="h")
points(0:max(degree(bicimad)),degree_dist,pch=20)
```
We can see that both plots are quite similar, i.e. the distributions are skewed to the left because of the large number of edges of the network.

Finally, we obtain the number of components of the generated network.

```{r}
components(random_graph_ER)
```
The number of components of the network is 1 as well as the real Bicimad network.

Now, we generate a network from the Gilbert model. We need to find a value of $c$ in order to obtain a probability $p = \frac{c}{N}$ which satisfies the number of edges of the real network. This value is 77.

```{r}
set.seed(10)
p <- 77/N
random_graph_G <- sample_gnp(n=N,p=p)
random_graph_G
```
We can see that the number of egdes is 10082 with a value for $c$ equals to 77, approximately the number of edges of the real bicimad network ($L =10000$). Now, we follow the same procedure as previously. First, we obtain a plot with circular layout.

```{r}
V(random_graph_G)$color <- "deepskyblue2"
plot.igraph(random_graph_G,layout=layout_in_circle,
            main="Gilbert network",
            vertex.size=8,vertex.label.cex=0.6)
```

As expected, again we cannot appreciate the connections between the nodes because of the large value of $L$. We check de degree distributions of the real and generated networks.

```{r}
deg_G <- degree(random_graph_G)
deg_dis_G <- degree_distribution(random_graph_G)
par(mfrow=c(2,2))
plot(0:max(deg_G),deg_dis_G,col="deepskyblue2",
     main="D. distribution of Gilbert network",
     xlab="Degree",ylab="Frequency",type="h")
points(0:max(deg_G),deg_dis_G,pch=20)
...
points(0:max(degree(bicimad)),degree_dist,pch=20)
```
Again, the degree distributions are skewed to the left. Finally, we obtain the number of components of the generated network.

```{r}
components(random_graph_G)
```

The number of components is 1, the same number of components than the real network.

## Check wheter your network has small world property using classical random graph models

For this step we will consider our network as a simple graphs, i.e., undirected and with no loops or multi-edges. Then we will generate a network with small world property using the Watts-Strogatz model. For that we will need to fix the order N, the number of neighbors r (which has de largest effect) and the rewiring probability p.

```{r}
bicimad_und=as.undirected(bicimad,mode = "collapse")
```

Therefore we will take a sample with 262 nodes (As the original bicimad network) and set the r as 40 neighbors, which is approximately close to the number of neighbors per node of the real network as we have seen through all the analysis.

```{r}
set.seed(10)
SW_1 <- sample_smallworld(1,262,40,0.05)
SW_1
```

Indeed we can see that the total number of links is 10480, which is pretty close to the 10000 that the real network has. We can also plot the degree distribution of the networt.

```{r}
par(mfrow=c(1,2))
deg_SW_1 <- degree(SW_1)
deg_dis_SW_1 <- degree_distribution(SW_1)
plot(0:max(deg_SW_1),deg_dis_SW_1,col="green",
     main="Degree distribution of the first small world network"
     ,xlab="Degree",ylab="Frequency",type="h")
deg_bicimad <- degree(bicimad)
deg_dis_bicimad <- degree_distribution(bicimad)
plot(0:max(deg_bicimad),deg_dis_bicimad,col="orange",
     main="Degree distribution of bicimad network"
     ,xlab="Degree",ylab="Frequency",type="h")

```

Again, the results obtained are actually pretty close to the original network, where the degree was also concentrated in the right side of the plot. However, the real network has larger variance for the degree.

```{r}
a=c(mean_distance(SW_1),mean_distance(bicimad_und))
b=c(diameter(SW_1),diameter(bicimad_und))
c=c(edge_density(SW_1),edge_density(bicimad_und))
d=c(transitivity(SW_1),transitivity(bicimad_und))
resume=data.frame("Mean_distance"=a,"Diameter"=b,"Edge Density"=c,"transitivity"=d) 
rownames(resume)=c("Small World Network","Bicimad Network")
resume
```

Analyzing those results we can see that there are a few differences. Specially in the mean distance and the transitivity, were we can see that indeed for the network generated it is almost 2 times more locally dense.

Now we will asses if the bicimad network has the small world property or not. For that we will be generating different networks with the same properties as the real network. For that we will using a classical model generator such us the Galbert model. We will be fixing both the number of nodes and the probability so the number of edges is as similar as possible to the real network.
...
for (i in 1 : n_trials){
  ER_graph <- sample_gnp(n=N,p=p)
  num_tran[i] <- transitivity(ER_graph)
  num_ave_path_length[i] <- mean_distance(ER_graph)
}
```

Now we compute the transitivity of the real network so we can compare it with the ones from all the networks generated. 

```{r}
transitivity(bicimad,weights=rep(1,262))
dist_bicimad <- distances(bicimad,weights=NULL)
mean(dist_bicimad[lower.tri(dist_bicimad,diag=FALSE)])
```

We can then plot histograms with the results from the networks generated and see their differences with the real network. For assessing that the bicimad network has the small-world behavior the observed clustering coefficients should be larger than those of the random ones, whereas the average length must remain roughly the same.

```{r}
par(mfrow=c(2,1))
hist(num_tran,col=color_1,
     xlab="Clustering coefficients",ylab="Density",main="Bicimad is 0.2795")
hist(num_ave_path_length,col=color_2,
     xlab="Average path length",ylab="Density",main="Bicimad is 2.4031")
```

As it can be seen, the clustering coefficient for the real network is much larger than those for the ones generated at random. Furthermore, the average path length is also higher but not that much. Therefore we cans ay that the bicimad network might have small world properties.
