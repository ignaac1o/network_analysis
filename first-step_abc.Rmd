...
As we mentioned in the first submission, the network has been built from two different dataset. The data included in them contains several variables which will define the vertexes and the edges of the network. These are the attributes of all the **directed** network:
 ...
 - *longitude (v/c)*: Longitude of the station in WGS84 format (v), which is a character (c).
 - *latitude (v/c)*: Latitude of the station in WGS84 format, which is a character (c).
 - *travel_time (e/n)*: Time of the entire trip (e), which is a number (n).
 - *user_type (e/n)*: Type of user (1: annual subscriber, 2: casual users, 3: employers) (e), which is a number (n).
 - *ageRange (e/n)*: Range of user age (e), which is a number (n).
 - *unplug_hourTime (e/c)*: Specific hour time in which the bike was unplugged (e), which is a charcter (c).
 - *zip_code (e/c)*: Zip code of the user (e), which is a character (c).
...
The previous two outputs show the label of the vertices attributes as well as the values which they contain. Specifically, the attributes are:
 ...
 - The lattitude and longitude where the station are.
...
head(E(bicimad)$"user_type")
head(E(bicimad)$"ageRange")
head(E(bicimad)$"unplug_hourTime")
head(E(bicimad)$"zip_code")
```
...
There are exactly 457 loops in the data. 
...
It seems difficult to identify the presence of groups in the graph, since the stations are well distributed over the city. However, the graph serves to see the loops mentioned and the multiple connections between stations.
...
We notice that the distribution is similar for al the cases. It means that there are approximately the same number of users that plug and unplug the bicycle in all the stations.

Furthermore, we can note that the degree distribution is slightly positive skewed for the three cases. Hence, there are some areas where the use of bicycles is high, i.e. there are some stations which present a high activity.
