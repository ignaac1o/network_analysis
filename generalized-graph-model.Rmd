...
Once we have applied the classical random graph models to our dataset, we are ready to describe the generalized random graph models. Generating a generalized random graph network with the same order (262 vertices) and with the same degree distribution than the bicimad network.

```{r}
deg_bicimad <- degree(bicimad)
G_bicimad <- sample_degseq(deg_bicimad, method="vl")
G_bicimad
```

As we can see in the description of the graph above, there are 262 vertices and 1000 edges, same as in the original, being the degree sequence equal in both networks. 

```{r}
par(mfrow=c(1,2))
deg_bicimad <- degree(bicimad)
deg_dis_bicimad <- degree_distribution(bicimad)
plot(0:max(deg_bicimad), deg_dis_bicimad, col=pal[1],
     main="Degree distribution of the bicimad",
     xlab="Degree", ylab="Frequency", type="h")
points(0:max(deg_bicimad), deg_dis_bicimad, pch=20)
deg_G_bicimad <- degree(G_bicimad)
deg_dis_G_bicimad <- degree_distribution(G_bicimad)
plot(0:max(deg_G_bicimad), deg_dis_G_bicimad, col=pal[1],
     main="Degree distribution of the generated",
     xlab="Degree", ylab="Frequency", type="h")
points(0:max(deg_G_bicimad), deg_dis_G_bicimad, pch=20)
```

The following charts compare the original network with the one we have just generated:

```{r fig.width=8, fig.height=8, fig.align='center'}
V(G_bicimad)$color <- pal[1]
par(mfrow=c(2,2))
plot.igraph(bicimad,layout=layout_as_tree(bicimad),
            main="Bicimad network",
            vertex.size=5, 
            edge.width=0.2, 
            edge.arrow.size=0, 
            edge.arrow.width=0,
            vertex.label.cex=0.5)
plot.igraph(G_bicimad,layout=layout_as_tree(bicimad),
...
plot.igraph(bicimad,layout=layout.norm(as.matrix(coordinates)),
            ...
            edge.arrow.width=0,
            vertex.label.cex=0.5)
plot.igraph(G_bicimad,layout=layout.norm(as.matrix(coordinates)),
            main="Generated network",
            vertex.size=5, 
            edge.width=0.2, 
            edge.arrow.size=0, 
...
In order to see the similarities of the original and the generated networks, we are going to show the main characteristics:

- Average path length:

```{r}
c(mean_distance(bicimad), mean_distance(G_bicimad))
```

- Diameter:

```{r}
c(diameter(bicimad, weights=rep(1, 10000)), diameter(G_bicimad))
```

- Density:

```{r}
c(edge_density(bicimad), edge_density(G_bicimad))
```

- Clustering coefficient:

```{r}
c(transitivity(bicimad, weights=rep(1,10000)), transitivity(G_bicimad))
```

- Number of components and size of the networks components:
...
```{r}
components(G_bicimad)
```

### Assessing the number of communities

For assessing the number of communities, we generate random graphs according to the the Erdös and Rényi model. We generate 500 networks with N=262 vertices and L=10,000 edges, then we calculate the same community detection methods we have executed previously.
...
options(warn=-1)
for (i in 1 : n_trials){
  ER_bicimad <- sample_gnm(n=N,m=L)
  cl_ER_bicimad_fg <- cluster_fast_greedy(ER_bicimad)
  num_comm_ER_fg[i] <- length(cl_ER_bicimad_fg)
  cl_ER_bicimad_lo <- cluster_louvain(ER_bicimad)
  num_comm_ER_lo[i] <- length(cl_ER_bicimad_lo)
  cl_ER_bicimad_lp <- cluster_label_prop(ER_bicimad,
                    initial=as.vector(membership(cl_ER_bicimad_lo)))
  num_comm_ER_lp[i] <- length(cl_ER_bicimad_lp)  
  cl_ER_bicimad_wa <- cluster_walktrap(ER_bicimad)
  num_comm_ER_wa[i] <- length(cl_ER_bicimad_wa)
}
options(warn=0)
```

The next step is to calculate the same graph partitions 100 times but using a generalized random graphs based on the degree sequence:
...
for (i in 1 : n_trials){
  G_bicimad <- sample_degseq(deg_bicimad, method="vl")
  cl_G_bicimad_fg <- cluster_fast_greedy(G_bicimad)
  num_comm_G_fg[i] <- length(cl_G_bicimad_fg)
  cl_G_bicimad_lo <- cluster_louvain(G_bicimad)
  num_comm_G_lo[i] <- length(cl_G_bicimad_lo)
  cl_G_bicimad_lp <- cluster_label_prop(G_bicimad,
                    initial=as.vector(membership(cl_G_bicimad_lo)))
  num_comm_G_lp[i] <- length(cl_G_bicimad_lp) 
  cl_G_bicimad_wa <- cluster_walktrap(G_bicimad)
  num_comm_G_wa[i] <- length(cl_G_bicimad_wa)
}
options(warn=0)
```

Then, we can compare the results obtained with each community detection with barplots:
...
In the label propagation case, as it occurred in the community detection section, there is just one proposed cluster, then we avoid to plot the barplot that compares the number of groups.
...
1. The algorithm *fast greedy* in the section community detection selects 4 graph partitions, nevertheless the results above shows that 4 has a low frequency. Then it could be not the best graph partitioning.

2. The *louvain* approach selects 3 graph partitions, nevertheless the results above shows that this option is not even considered.

3. The *label propagation* method select 2 groups, although we have not considered this option since the modularity was really small. In this case, the groups proposed here are just one.

4. The *walktrap* method select 7 groups in the original data, for the generated graph models we have obtained that the best would be 4 graph partitions. Therefore we cannot accept 7 groups as the best since the frequency value associated is very small.
